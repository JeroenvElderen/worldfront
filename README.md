...
The game includes the first-time city flow, local autosave, starter company and taxi, map base marker, HUD, and navigable game sections. A random taxi request arrives every 30 seconds (up to six open offers). Accepted fares use Mapbox's `driving-traffic` directions, so the displayed route follows drivable roads and current traffic-aware restrictions rather than drawing a straight line; the taxi marker travels that route using the live ETA. Hiring, purchases, travel agencies and multiple save-slot UI are reserved for future milestones. The IndexedDB save schema uses a named `autosave` key so additional slots can be introduced without replacing the persistence layer.
