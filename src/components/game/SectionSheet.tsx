...
export function SectionSheet({ section, vehicles, drivers, loans, cash, onClose, onReset, onBuyTaxi, onLeaseTaxi, onTakeLoan, onSellVehicle, onSetDriverShift, onToggleAccessory }: SectionSheetProps) {
  ...
  if (section === 'fleet') return <section className="section-sheet fleet-sheet game-panel">
    ...
    <div className="fleet-list">{vehicles.map((vehicle) => { const model = getModel(vehicle); const serviceModel = vehicle.modelId ? getServiceVehicleModel(vehicle.modelId) : undefined; const driver = drivers.find((candidate) => candidate.id === vehicle.driverId); return <button className={`fleet-card ${customizingVehicleId === vehicle.id ? 'selected' : ''}`} onClick={() => setCustomizingVehicleId(vehicle.id)} key={vehicle.id}>{model ? <img className="brand-logo" src={model.logoUrl} alt={`${model.brand} logo`} /> : <span className="brand-fallback">{serviceModel?.icon ?? '◆'}</span>}<div><strong>{vehicle.name}</strong><small>{vehicle.condition}% condition · {vehicle.status === 'on-job' ? 'Driving' : vehicle.serviceTrip ? `Going to ${vehicle.serviceTrip.label}` : 'Available'} · {vehicle.ownership ?? 'owned'}</small><div className="meter-row"><span>⛽ {Math.round(vehicle.fuel)}%</span>{driver && <span>😴 {Math.round(driver.fatigue)}%</span>}</div></div></button> })}</div>
