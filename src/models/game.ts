...
export type ResearchId = 'smart-dispatch' | 'predictive-demand' | 'autonomous-operations' | 'network-orchestration' | 'demand-twin' | 'quantum-routing' | 'sentient-dispatch' | 'preventive-diagnostics' | 'modular-workshops' | 'fleet-telemetry' | 'performance-lab' | 'exclusive-fleet' | 'carbon-chassis' | 'apex-engineering' | 'eco-routing' | 'rapid-charging' | 'circular-fleet' | 'battery-reclamation' | 'solar-depots' | 'hydrogen-grid' | 'zero-impact-network' | 'prefab-depots' | 'regional-hubs' | 'global-network' | 'mega-depots' | 'continental-corridors' | 'orbital-logistics' | 'planetary-network'
...
export type DemandLevel = 'quiet' | 'steady' | 'busy' | 'surging'
export interface DemandHotspot { id: string; cityId: string; name: string; coordinates: Coordinates; score: number; level: DemandLevel; reason: string; category: string }
