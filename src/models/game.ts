...
export type PostalServiceLevel = 'standard' | 'express' | 'business'
...
export interface PostalRoutePlan { plannedHours: number; radiusKm: number; loadPercent: number; serviceLevel: PostalServiceLevel; seed: number }
export interface PostalRoute { stops: PostalStop[]; startedAt: string; arrivesAt: string; reward: number; plannedHours?: number; distanceKm?: number; parcelCount?: number; loadPercent?: number; serviceLevel?: PostalServiceLevel; radiusKm?: number }
export interface PostalPerformance { completedRounds: number; deliveredParcels: number; revenue: number; distanceKm: number; onTimeRounds: number }
export interface PostalDepot { id: string; cityId: string; name: string; coordinates: Coordinates; level: number; openedAt: string }
...
export interface FerryRouteOption { id: string; name: string; originName: string; destinationName: string; originCoordinates: Coordinates; destinationCoordinates: Coordinates; routeCoordinates: Coordinates[]; distanceKm: number; durationMinutes?: number; source: 'openstreetmap' | 'curated'; harbourId?: string }
export interface DiscoveredFerryRoute extends FerryRouteOption { cityId: string; discoveredAt: string }
export interface PurchasedHarbour { id: string; name: string; coordinates: Coordinates; purchasedAt: string }
