...
import type { Company, Coordinates, Passenger, TaxiJob, Vehicle } from '../models/game'

const locations = [
  ['Central Station', 0.012, 0.006], ['Riverside Hotel', -0.015, 0.008],
  ['Market Square', 0.009, -0.011], ['City Hospital', -0.008, -0.014],
...
export function createJobOffers(cityId: string, id = crypto.randomUUID): { jobs: TaxiJob[]; passengers: Passenger[] } {
  const city = getCity(cityId)
  if (!city) return { jobs: [], passengers: [] }
  const passengers = passengerNames.slice(0, 4).map((name, index) => ({ id: id(), name, partySize: (index % 3) + 1 }))
...
export function acceptJobState(jobs: TaxiJob[], vehicles: Vehicle[], jobId: string) {
  if (jobs.some((job) => job.status === 'accepted')) return null
  ...
  return {
    jobs: jobs.map((candidate) => candidate.id === jobId ? { ...candidate, status: 'accepted' as const } : candidate),
    vehicles: vehicles.map((candidate) => candidate.id === vehicle.id ? { ...candidate, status: 'on-job' as const } : candidate),
  }
}

export function completeJobState(company: Company, jobs: TaxiJob[], vehicles: Vehicle[], jobId: string) {
  ...
  return {
    ...
    vehicles: vehicles.map((candidate) => candidate.status === 'on-job' ? { ...candidate, status: 'available' as const } : candidate),
  }
}
