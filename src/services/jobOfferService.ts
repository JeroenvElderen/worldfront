...
async function fetchMapboxPlaces(city: City, signal?: AbortSignal): Promise<MapboxPlace[]> {
  ...
  const countryCities = cities.filter((candidate) => candidate.countryCode === city.countryCode)
  const responses = await Promise.all(countryCities.flatMap((searchCity) => placeSearches.map(async (search) => {
    const url = new URL('https://api.mapbox.com/search/searchbox/v1/forward')
    url.searchParams.set('q', `${search} in ${searchCity.name}`)
    url.searchParams.set('access_token', mapboxToken)
    url.searchParams.set('proximity', searchCity.coordinates.join(','))
  ...
  const places = responses.flatMap(({ features = [] }) => features.flatMap((feature): MapboxPlace[] => {
    ...
    return [{ id, name: name.trim(), coordinates }]
  }))
...
function findMapboxPlaces(city: City, signal?: AbortSignal) {
  const cached = placeCache.get(city.countryCode)
  if (cached) return cached
  const request = fetchMapboxPlaces(city, signal).catch((error) => {
    placeCache.delete(city.countryCode)
    throw error
  })
  placeCache.set(city.countryCode, request)
  return request
}
