...
const generationTaxi = selectJobGenerationTaxi(state)
  ...
  buyHarbour: (route) => set((state) => {
    ...
    const connectedToOwnedHarbour = allFerryRoutes().some((candidate) =>
    ...
    return {
      company: { ...state.company, cash: state.company.cash - cost },
      purchasedHarbours: [...(state.purchasedHarbours ?? []), { id, name: route.originName, coordinates: route.originCoordinates, purchasedAt }],
      financialTransactions: addTransactions(state.financialTransactions, transaction('expansion', `Purchased ${route.originName}`, -cost, undefined, purchasedAt)),
  ...
  createFerryRoute: (ferryRoute) => set((state) => {
    const from = getCity(state.activeCityId ?? state.startingCityId, state.customCities)
    if (!from || !(state.purchasedHarbours ?? []).some((harbour) => harbour.id === `harbour:${ferryRoute.originCoordinates.map((part) => part.toFixed(4)).join(',')}`) || (state.transportRoutes ?? []).some((route) => route.mode === 'ferry' && route.sourceRouteId === ferryRoute.id)) return state
    return {
