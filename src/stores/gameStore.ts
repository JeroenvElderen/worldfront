...
export const useGameStore = create<GameState & GameActions>()(persist((set) => ({
  ...
  setIdleRoamRoute: (vehicleId, startedAt, waypoints) => set((state) => ({
    vehicles: state.vehicles.map((vehicle) => vehicle.id === vehicleId && vehicle.idleRoam?.startedAt === startedAt
      ? { ...vehicle, idleRoam: { ...vehicle.idleRoam, waypoints } }
      : vehicle),
  })),
  ...
  refreshJobs: async () => {
    ...
    try {
      ...
      const generated = await generateJobOffers(searchArea, 1, state.jobRequestHistory ?? [], maxDistanceKm, undefined, taxiPositions, (state.activeEvent?.fareMultiplier ?? 1) * cityDemandMultiplier(state.cityEconomies.find((economy) => economy.cityId === city.id)) * (state.worldCondition?.demandMultiplier ?? 1) * marketingDemandMultiplier(state.brandStrategy ?? defaultBrandStrategy) * fareMultiplier(state.brandStrategy?.fareStrategy ?? 'standard') * Math.max(.55, 1 - (state.competitors ?? []).filter((competitor) => competitor.relationship === 'rival').reduce((sum, competitor) => sum + competitor.marketShare, 0) / 200), state.company?.foundedAt)
      set((latest) => {
  ...
  tickJobs: () => set((state) => {
    ...
    vehicles = vehicles.map((vehicle) => {
      if (vehicle.type !== 'taxi' || vehicle.status !== 'available' || !vehicle.driverId || vehicle.fuel <= 20 || vehicle.condition < 30) return vehicle
      const driver = drivers.find((candidate) => candidate.id === vehicle.driverId)
      if (!driver || driver.status !== 'available') return vehicle
      if (vehicle.idleRoam && new Date(vehicle.idleRoam.arrivesAt).getTime() > now) return vehicle
      const position = vehicle.idleRoam ? idleRoamPosition(vehicle.idleRoam, now) : vehicle.position
      const city = getCity(vehicle.cityId, state.customCities)
      return city ? { ...vehicle, position, idleRoam: createIdleRoam({ ...vehicle, position }, city.coordinates, now) } : vehicle
    })
